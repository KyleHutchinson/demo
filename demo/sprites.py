...
class Pow(Sprite):
    def __init__(self, game, plat):
        ...
        #changed the blue particle powerup to be a spring
        self.image = self.game.spritesheet.get_image(420, 1558, 100, 65)
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
...
class Cactus(Sprite):
    ...
    def update(self):
        ...
        if not self.game.platforms.has(self.plat):
            self.kill()
#add Fake platform class
class Fake(Sprite):
    def __init__(self, game, x, y):
        # allows layering in LayeredUpdates sprite group
        self._layer = PLATFORM_LAYER
        # add Platforms to game groups when instantiated
        self.groups = game.all_sprites, game.fakes
        Sprite.__init__(self, self.groups)
        self.game = game
        #makes fake platforms broken ground
