...
class Game:
    ...
    def new(self):
        ...
        #create a fake platform group
        self.fakes = pg.sprite.Group()
        # create clouds group
    ...
    def update(self):
        ...
        # check to see if player collides with platorm - if falling
        ...
        if self.player.rect.top <= HEIGHT / 4:
            ...
            for plat in self.platforms:
                ...
                if plat.rect.top >= HEIGHT + 40:
                    plat.kill()
                    self.score += 10
        # if player reach top 1/4 of screen with fake platforms
        if self.player.rect.top <= HEIGHT / 4:
        ...
        while len(self.platforms) < 8:
            ...
            Platform(self, random.randrange(0,WIDTH-width), 
                            random.randrange(-75, -30))
        #generates new fake platforms
        while len(self.fakes) < 6:
            width = random.randrange(50, 100)
            ''' removed widths and height params to allow for sprites '''
            """ changed due to passing into groups through sprites lib file """
            # p = Platform(self, random.randrange(0,WIDTH-width), 
            #                 random.randrange(-75, -30))
            Fake(self, random.randrange(0,WIDTH-width), 
                            random.randrange(-75, -30))
            # self.platforms.add(p)
            # self.all_sprites.add(p)
    def events(self):
